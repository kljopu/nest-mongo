...
export class MongoDbUnitOfWork implements UnitOfWorkPort {
  constructor(
    @InjectConnection()
  ...
  getSession(correlationId: string): ClientSession {
    const session = this.sessions.get(correlationId);
    if (!session) {
      throw new Error(
        'session not found. Incorrect correlationId or transaction is not started. To start a transaction wrap operations in a "execute" method.',
      );
    }
    return session;
  }

  getMongooesRepository<Entity>(
    entity: EntityTarget<Entity>,
    correlationId: string,
  ): Repository<Entity> {
    const queryRunner = this.getQueryRunner(correlationId);
  ...
  private async rollbackTransaction<T>(correlationId: string, error: Error) {
    const session = this.getSession(correlationId);
    try {
